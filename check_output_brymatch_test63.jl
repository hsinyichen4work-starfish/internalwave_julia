...
# each day's bry file is named after that day at hour 00, e.g. 2022090100
...
# no dbry flux files for this (wo_dbry) run, so there's no flux_specs/flux
# check here — unlike check_output_brymatch_hpc.jl's dbry-forced runs.
...
## Runs all 4 boundaries for one calendar day: builds that day's bry/his_files
# paths, skips cleanly if a day's input is missing (e.g. a gap in the his
# output), then delegates to process_boundary per boundary. This is the unit
# of work handed out to worker processes by pmap below — one call per day,
# each call producing that day's own set of figures (datestr/datestr_save
# below always come from that day's own bry_time), so the figures show one
# day at a time even though many days now run at once.
@everywhere function process_day(date::Date)
    datestr_files = Dates.format(date, "yyyymmdd")
    bry_fname = bry_fname_for(date)

    if !isfile(bry_fname)
        println("!! skipping $datestr_files — missing bry file ($bry_fname)")
        return nothing
    end
