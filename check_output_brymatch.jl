...
include("/home/hsinyi/Documents/Julia/function/plotting_fun.jl")   # topdown_axis3, plot_curvilinear! (surface!-as-pcolormesh trick, reused below for time/depth)
...
bry_fname = "/home/hsinyi/roms_data/bry_63/roms_bry_900m_2022090100.nc"
...
his_files = sort(filter(f -> occursin("roms_his", basename(f)) && endswith(f, ".nc"),
...
# boundary from the loop at the bottom of this file, so only one boundary's
# worth of his-file arrays is ever live in memory at a time — the previous
# boundary's arrays are local to the previous call and get GC'd afterwards.
function process_boundary(boundary::String)
...
## flux check: up_east/up_west/vp_north/vp_south are the HF baroclinic energy
# flux (rho0-normalized) at each open boundary, read straight from
# dbry_fname — these already live on the boundary line (eta_rho for
# east/west, xi_rho for north/south), no u/v extraction needed. A sign flip
# here means the flux — and with it the dynamic (radiation-type) boundary
# condition switch — has changed between incoming and outgoing.
flux_specs = [
    ("up_east",  "east"),
    ("up_west",  "west"),
    ("vp_north", "north"),
...
for (varname, boundary) in flux_specs
    ...
    # date is wrong: the raw value for this "2022090100" file is 10470.0,
    # and t_ref (1994-01-01) + 10470 days = 2022-09-01 (matches the
    # filename) whereas 1900-12-31 + 10470 days = 1929-08-31 (nonsense).
    # So bry_time here is actually a plain day-count since t_ref, same as
    # every other bry_time in this script — read via `.var[:]` to bypass
    # NCDatasets' (mistaken) CF auto-decoding.
